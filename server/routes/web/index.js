...
module.exports = app=>{
    ...
    router.get('/news/init',async(req,res)=>{
        ...
        const newsList = newsTitles.map(title =>{
            const randomCats = cats.slice(0).sort((a,b)=>Math.random()-0.5) //这里slice(0)表示复制一份，不会改变原来的结果
            return{
                categories:randomCats.slice(0,2),
                title:title
            }
        })
        await Article.deleteMany({})    //s删除Article里的所有数据
    ...
    router.get('/news/list',async(req,res)=>{
        ...
        //mongoose   aggregate   聚合查询
        const cats = await Category.aggregate([
            {   //$match类似于mysql的where，过滤   parent:parent._id   模型里的字段名：上面获取的id
                $match:{ parent:parent._id}},
            {
                $lookup:{
                    from:'articles',         //是模型的复数   需要关联的表
                    localField:'_id',       //表需要关联的键
                    foreignField:'categories',      //matching key
                    as:'newsList'
                }
            },
            {
                $addFields:{        //添加字段、修改字段 
                    newsList:{
                        $slice:['$newsList',5]          //这样每个newList都只有五个了
                    }
                }
            }
        ])
        ...
        cats.unshift({
            ...
            newsList:await Article.find().where({
                    categories:{$in:subCats}        //$in 数组array查询 $in
            }).populate('categories').limit(5).lean()
        })

        cats.map(cat=>{         //因为每一个文章对应的可能不止一个分类
...
router.get('/heroes/init',async(req,res)=>{
 ...
 router.get('/videoes/list',async(req,res)=>{
    ...
    cats.unshift({
        name:'精彩栏目',
        videoesList:await Video.find().where({
                categories:{$in:subCats}        //$in 数组array查询 $in
        }).limit(4).lean()
    })
    res.send(cats)
...
router.get('/articles/:id',async(req,res)=>{
  ...
  //视频详情
  router.get('/videoes/:id',async(req,res)=>{
    const data = await Video.findById(req.params.id).populate('categories')
    .lean()
      res.send(data)
})
